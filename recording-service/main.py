...
def _build_recordings_tree(root: Path) -> dict[str, Any]:
    ...
    node["children"] = _sort_recordings_children(node["children"])
    return node


def _sort_recordings_children(children: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Order recordings tree children: folders first in ascending alphabetical
    order, then files in descending alphabetical order. Recording file names
    start with "<YYYY>-<MM>-<DD> <HH>-<MM>", so descending order puts the
    newest recording on top."""
    folders = sorted(
        (c for c in children if c["type"] == "folder"),
